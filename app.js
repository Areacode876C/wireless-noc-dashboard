function calculateHealth(site) {
    ...
    if (site.signalStrength < -85) {
        score -= 15;
    }

    if (site.signalStrength < -100) {
        score -= 20;
    }
    ...
    if (site.droppedCalls > 4) {
        score -= 15;
    }

    if (site.droppedCalls > 10) {
        score -= 20;
    }

    if (site.downloadMbps < 100) {
        score -= 10;
    }
...
function identifyIssueDomain(site) {
    ...
    const poorSignal =
        site.signalStrength < -100;

    const highLatency =
        site.latency > 120;

    const lowThroughput =
        site.downloadMbps < 100;

    const highDrops =
        site.droppedCalls > 10;

    if (poorSignal && highDrops) {
        return {
            domain: "Radio Access Network",
            issue:
                "Poor coverage or radio-performance condition",
            recommendation:
    ...
    if (lowThroughput && site.connectedDevices > 1200) {
        return {
            domain: "Capacity",
            issue:
                "Possible congestion during elevated utilization",
            recommendation:
    ...
    if (highDrops) {
        return {
            ...
            issue:
                "Elevated simulated dropped-call count",
            recommendation:
    ...
    return {
        ...
        issue:
            "Site KPIs are within this demo's expected range",
        recommendation:
        ...
        <article class="site-card ${status.toLowerCase()}">
            ...
            <div class="kpi-grid">
                ...
                <div>
                    ...
                    <strong>
                        ${site.signalStrength} dBm
                    </strong>

                    <small>
                        ${formatSignal(site.signalStrength)}
                    </small>
                </div>
...
function updateSummary(data) {
    ...
    document.getElementById(
        "total-sites"
    ).textContent = analyzedSites.length;
    ...
    document.getElementById(
        "average-latency"
    ).textContent = averageLatency;
}

function displaySites(filter = "All") {

    const filteredData =
        networkData.filter(site => {
    ...
    const siteGrid =
        document.getElementById("site-grid");
